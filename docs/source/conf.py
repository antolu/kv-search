...
project = "kv-search"
author = "Anton Lu"
version = kv_search.__version__

copyright = f"{datetime.datetime.now().year}, Anton Lu"  # noqa: A001
...
def hijack_module_name_replacement() -> None:
    ...
    import kv_search._mod_replace

    for mod_name in list(sys.modules):
        if mod_name == "kv_search._mod_replace":
            continue
        if mod_name == "kv_search" or mod_name.startswith("kv_search."):
            del sys.modules[mod_name]

    kv_search._mod_replace.replace_modname = lambda *_: None  # noqa: SLF001
